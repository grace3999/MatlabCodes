...
%folder path is: F:\Schindler Lab\Data\DRL\TreatmentGroup\Animal#\Behavior\DRL
...
pathname1='F:\Schindler_Lab\Data\DRL\';
...
for i=1:length(DRL);
    for j=1:length(DRL(i).ratIDlist);
        ...
        %example: F:\Schindler Lab\Data\DRL\Control\26\Behavior\DRL\
...
for i=1:length(DRL);
    ...
    DRL(i).Animal(j).behavior=nan(3000,2,length(DRL(i).Animal(j).behavsessionlist));
        ...
        %fileName= char(strcat(pathname1, group(i), '\', DRL(i).ratIDlist(j), '\Behavior\DRL\', DRL(i).Animal(j).behavsessionlist{k})); %filename input will be sessionlist(n=1:lenght(sessionlist))
...
save ('drlstructuretwo.mat', 'DRL', '-mat')
