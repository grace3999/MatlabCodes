...
for i=1:length(listing);
    if isstrprop(listing(i).name(1), 'digit'); %1 if true, 0 if false 
            %.name(1) is indexing into first value in string only 
        ratIDlist=[ratIDlist; listing(i).name];%if filename starts with... 
...
for i=1:length(ratIDlist);
    ...
    listing2=dir(char(strcat(pathname1, ratIDlist(i), '\Behavior\DRL\')));
    ...
    for j=1:length(listing2);
        if isstrprop(listing2(j).name(1), 'digit'); %1 if true, 0 if false
            sessionlist=[sessionlist; listing2(j).name];%if filename...
